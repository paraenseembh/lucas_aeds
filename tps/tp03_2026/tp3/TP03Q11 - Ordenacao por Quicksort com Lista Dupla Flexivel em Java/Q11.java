...
public class Q11 {
    ...
    static class Node {
        Restaurante dado;
        Node prev, next;
        Node(Restaurante r) { this.dado = r; }
    }
    ...
    static class ListaDupla {
        ...
        Node[] toArray() {
            ...
            Node cur = head;
            for (int i = 0; i < size; i++) { arr[i] = cur; cur = cur.next; }
            return arr;
        }
    }
    ...
    static int partition(Node[] arr, int left, int right) {
        Restaurante pivot = arr[left].dado;
        int i = left - 1, j = right + 1;
        while (true) {
            do { i++; comparacoes++; } while (compara(arr[i].dado, pivot) < 0);
            do { j--; comparacoes++; } while (compara(arr[j].dado, pivot) > 0);
            if (i >= j) return j;
            Restaurante tmp = arr[i].dado; arr[i].dado = arr[j].dado; arr[j].dado = tmp;
            movimentacoes += 3;
        }
    }

    static void quicksort(Node[] arr, int l, int r) {
        if (l < r) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Node cur = lista.head;
        while (cur != null) {
            System.out.println(cur.dado.formatar());
            cur = cur.next;
        }
