...
/* Pilha flexivel (LIFO) - push/pop from end of array */
class Pilha {
    private Restaurante[] dados;
    private int topo;

    public Pilha(int capacidade) {
        dados = new Restaurante[capacidade];
        topo = -1;
    }
    ...
    public Restaurante get(int i) { return dados[i]; }
}
...
public class Q6 {
    public static void main(String[] args) throws Exception {
        ...
        while ((id = sc.nextInt()) != -1)
            if (mapa.containsKey(id)) pilha.push(mapa.get(id));
        ...
        for (int i = 0; i < n_ops; i++) {
            ...
            if (op.equals("I")) {
                id = sc.nextInt();
                if (mapa.containsKey(id)) { pilha.push(mapa.get(id)); movimentacoes++; }
            } else if (op.equals("R")) {
        ...
        for (int i = pilha.tamanho() - 1; i >= 0; i--)
            System.out.println(pilha.get(i).formatar());

        PrintWriter log = new PrintWriter("797798_pilha.txt");
