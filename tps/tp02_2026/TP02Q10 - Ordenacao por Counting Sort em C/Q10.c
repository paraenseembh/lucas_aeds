...
void counting_sort(Restaurante** arr, int n, Restaurante** out){
    if(n==0) return;
    int max_cap=0;
    for(int i=0;i<n;i++) if(arr[i]->capacidade>max_cap) max_cap=arr[i]->capacidade;
    int* count=(int*)calloc(max_cap+1,sizeof(int));
    for(int i=0;i<n;i++) count[arr[i]->capacidade]++;
    for(int i=1;i<=max_cap;i++) count[i]+=count[i-1];
    for(int i=n-1;i>=0;i--){
        out[--count[arr[i]->capacidade]]=arr[i];
        movimentacoes++;
    }
    free(count);
}
...
int main(){
    ...
    Restaurante* out[600];
    clock_t ini=clock();
    counting_sort(arr,n,out);
    clock_t fim=clock();
    double tempo=(double)(fim-ini)/CLOCKS_PER_SEC*1000.0;
    char buf[500];
    for(int i=0;i<n;i++){formatar_restaurante(out[i],buf);printf("%s\n",buf);}
    FILE* log=fopen("matricula_countingsort.txt","w");
    fprintf(log,"%s\t0\t%lld\t%.2f\n",MATRICULA,movimentacoes,tempo);
    fclose(log);
    return 0;
}
