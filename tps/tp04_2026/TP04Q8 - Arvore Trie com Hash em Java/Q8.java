...
public class Q8 {
    ...
    // ── Trie with HashMap children ──────────────────────────────────────────

    static class No {
        char c;
        Restaurante dado;
        HashMap<Character, No> filhos = new HashMap<>();
        No(char c) { this.c = c; }
    }
    ...
    static void inserir(Restaurante r) {
        ...
        for (char ch : r.nome.toCharArray()) {
            cur.filhos.putIfAbsent(ch, new No(ch));
            cur = cur.filhos.get(ch);
        }
        cur.dado = r;
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        PrintWriter log = new PrintWriter(new FileWriter(MATRICULA + "_arvore_trie_hash.txt"));
